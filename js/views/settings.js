import { exportDatabase, importDatabase, resetDatabase, validateBackup, DB_VERSION } from "../db.js";
...
export async function renderSettings(root) {
  ...
  root.innerHTML=`${pageHeader("Einstellungen","Personalisiere PersonalOS und verwalte deine lokalen Daten.")}
    ...
    <section class="card about"><img src="./assets/icons/icon.svg" alt="" width="48"><div><b>PersonalOS</b><small>Lokale Offline-PWA · Schema ${DB_VERSION}</small></div></section>`;
  ...
  root.querySelector("[data-import]").onchange=async e=>{try{
    const file=e.target.files[0];if(!file)return;const value=JSON.parse(await file.text());validateBackup(value);
    const mode=root.querySelector("[data-mode]").value;
    if(mode==="replace"&&!await confirmDialog("Replace löscht zuerst sämtliche aktuellen PersonalOS-Daten. Die geladene Sicherung wurde validiert. Wirklich fortfahren?","Daten ersetzen"))return;
    await importDatabase(value,mode);await state.load();toast("Import abgeschlossen");window.dispatchEvent(new HashChangeEvent("hashchange"));
  }catch(error){toast(error.message,"error");}finally{e.target.value="";}};
